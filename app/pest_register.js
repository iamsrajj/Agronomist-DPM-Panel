...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const registerForm = document.getElementById("pest_register");
    ...
    fetch("http://103.76.249.121:3000/pest_register", {
      ...
      .then((response) => {
        if (response.ok) {
          ...
          throw new Error("Pest already registered!");
      ...
      .then((data) => {
        ...
        alert("Pest registration successful!");
      })
      .catch((error) => {
        console.error("Disease registration failed:", error);
        // Handle error, such as displaying an error message to the user
        alert("Failed to register pest. Please try again later.");
      });
  });
});
