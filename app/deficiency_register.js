...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const registerForm = document.getElementById("deficiency_register");
    ...
    fetch("http://103.76.249.121:3000/deficiency_register", {
      ...
      .then((response) => {
        if (response.ok) {
          ...
          throw new Error("Deficiency already registered!");
      ...
      .then((data) => {
        ...
        alert("Deficiency registration successful!");
      })
      .catch((error) => {
        console.error("Disease registration failed:", error);
        // Handle error, such as displaying an error message to the user
        alert("Failed to register deficiency. Please try again later.");
      });
  });
});
